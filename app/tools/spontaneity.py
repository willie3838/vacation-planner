...
def spontaneous_recommendations(
    ...
    mood_or_preference: str = "relaxing and lowkey",
    radius: str = "walking distance",
) -> SpontaneousResponse:
    ...
    Returns:
        A SpontaneousResponse containing spontaneous, lowkey activities with Reddit consensus, costs, and best timing.
    """
    if not current_location or not current_location.strip():
        return SpontaneousResponse(
            status="error",
            error="Current location cannot be empty.",
            recovery_instruction="Ask the traveler where they currently are (neighborhood, street, or nearby landmark) before finding spontaneous spots.",
            suggested_action="PROMPT_USER_FOR_LOCATION",
        )
    ...
    recommendations: list[SpontaneousRecommendationItem] = []
